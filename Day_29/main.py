...
password_prompt_entry = None
password_prompt_label = None

# -----------------------Password prompt interface------------------------------
password_prompt = Tk()
password_prompt.title("Password Prompt")
password_prompt.config(padx=30, pady=30)
#password_prompt_canvas = Canvas(password_prompt, width=300, height=100)
#password_prompt_canvas.create_image(200, 200)


# -----------------------Check Password-----------------------------------------
def check_password():
    global password_prompt_entry, password_prompt_label
    entered_password = password_prompt_entry.get()
    if entered_password == "1234":
        password_prompt.destroy()
        main_program()
    else:
        password_prompt_entry.delete(0, 'end')
        password_prompt_label.config(text="Incorrect password", fg="red")


password_prompt_label = Label(password_prompt, text="Enter password:")
password_prompt_label.grid(column=0, row=1)

password_prompt_entry = Entry(password_prompt, show="*")
password_prompt_entry.grid(column=1, row=1)

password_submit_password = Button(password_prompt, text="Submit", command=check_password)
password_submit_password.grid(column=1,row=1)
...
# -----------------------Encryption key handling--------------------------------
    ...
    add_button.grid(column=2, row=6, ipadx=5, padx=(0,79))
    ...
    search_button.grid(column=2, row=7, padx=(0,24))
